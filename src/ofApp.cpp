...
//			cout << "deleting sprite: " << s->name << endl;
	...
	for (int i = 0; i < sprites.size(); i++) {
		...
		if (charge2) {
			if (sprites[i].age() > 0 && sprites[i].age() < 500)
				sprites[i].velocity *= 0;
			else if (sprites[i].age() < 5000) {
				glm::vec3 heading = glm::normalize(shipPos - sprites[i].trans);
				sprites[i].acceleration = heading * 5;
				sprites[i].velocity += sprites[i].acceleration;
			}
			else
			{
				sprites[i].acceleration *= 0;
			}
		}
...
void SpriteSystem::collideCheck2(BaseObject *ship) {
	for (int i = 0; i < sprites.size(); i++) {
		if ((sprites[i].trans.x > ship->trans.x && sprites[i].trans.x < ship->trans.x + ship->width && sprites[i].trans.y > ship->trans.y && sprites[i].trans.y < ship->trans.y + ship->height) ||
			(sprites[i].trans.x + sprites[i].width > ship->trans.x && sprites[i].trans.x + sprites[i].width < ship->trans.x + ship->width && sprites[i].trans.y > ship->trans.y && sprites[i].trans.y < ship->trans.y + ship->height) ||
			(sprites[i].trans.x + sprites[i].width > ship->trans.x && sprites[i].trans.x + sprites[i].width < ship->trans.x + ship->width && sprites[i].trans.y + sprites[i].height > ship->trans.y && sprites[i].trans.y + sprites[i].height < ship->trans.y + ship->height) ||
			(sprites[i].trans.x > ship->trans.x && sprites[i].trans.x < ship->trans.x + ship->width && sprites[i].trans.y + sprites[i].height> ship->trans.y && sprites[i].trans.y + sprites[i].height < ship->trans.y + ship->height)
			) {
			sprites[i].lifespan = 0;
			bam = true;
			//cout << "pop" << endl;
...
void Emitter::update() {
	...
	if (shoot == true && (time - lastSpawned) > (1000.0/rate)) {
		...
		if (rando) {
			trans.y = rand() % ofGetWindowHeight();
			if (rand() % 2) {
				trans.x = -50;
			}
			else {
				trans.x = ofGetWindowWidth() + 50;
			}
		}
		// spawn a new sprite
		Sprite sprite;
...
void ofApp::setup(){
	...
	if (defaultImage.load("images/enemy2.png")) {
		imageLoaded = true;
		spawn2->setChildImage(defaultImage);
	}
	else {
		ofLogFatalError("can't load image: images/enemy2.png");
		ofExit();
	}
	...
	spawn2->setPosition(ofVec3f(-50, ofGetWindowHeight() / 2.0, 0));
	spawn2->setLifespan(7000);
	spawn2->sys->charge2 = true;
	spawn2->rando = true;
	spawn2->shoot = true;

	
	gui.setup();
	gui.add(rate.setup("rate", 4, 0, 10));
	...
	emitter.setParticleRadius(2);
	//emitter.setLifespan(1);
	//emitter.setPosition(ofVec3f(100, 100, 0));
...
void ofApp::update() {
	...
	spawn2->setRate(erate/2);
	spawn2->setLifespan(elife * 1000);
	...
	spawn1->update();
	spawn2->sys->shipPos = glm::vec3(turret->trans.x + turret->width / 2.0, turret->trans.y + turret->height / 2.0, 1);
	spawn2->update();

	turret->sys->collideCheck(spawn1->sys);
	turret->sys->collideCheck(spawn2->sys);
	spawn1->sys->collideCheck2(turret);
	spawn2->sys->collideCheck2(turret);
	...
	if (turret->sys->boom)
	{
		...
		emitter.setPosition(turret->sys->explodePos);
		emitter.start();
	}

	if (spawn1->sys->bam)
	{
		spawn1->sys->bam = false;
		score -= 100;
		radio.load("sounds/bam.mp3");
		radio.play();

		emitter.setPosition(spawn1->sys->explodePos);
		emitter.start();
	}

	if (spawn2->sys->bam)
	{
		spawn2->sys->bam = false;
		score -= 100;
		radio.load("sounds/bam.mp3");
		radio.play();

		emitter.setPosition(spawn2->sys->explodePos);
		emitter.start();
	}
